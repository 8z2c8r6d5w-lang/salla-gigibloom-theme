...
/**
 * Whether a pickup branch can be selected.
 *
 * `pickup_readiness` is the richer, backend-computed verdict (INV-357) and wins whenever the API
 * sends it. Not every store's `GET /branches` returns it though, so we fall back to the original
 * `is_open` rule instead of treating a missing object as "not selectable" — otherwise every branch
 * in the list renders disabled and pickup cannot be chosen at all (SP-22202).
 */
const isBranchSelectable = (branch) => {
    if (!branch)
        return false;
    const selectable = branch.pickup_readiness?.selectable;
    return typeof selectable === 'boolean' ? selectable : branch.is_open !== false;
};
...
const getIntentSubtitle = (intent, toAddress, fromBranch) => {
    if (intent.type === 'branch') {
        const loc = [intent.branch_details?.country, intent.branch_details?.city, intent.branch_details?.district].filter(Boolean).join('، ');
        return loc ? `${fromBranch} ${loc}`.trim() : '';
    }
    if (intent.type === 'address' && intent.address_details) {
        const addr = [intent.address_details.country?.name, intent.address_details.city?.name, intent.address_details.district?.name].filter(Boolean).join('، ') || intent.address_details.short_address || '';
...
const isMapOriginIntent = (i) => !!i && i.type === 'address' && i.address_details?.lat != null && i.address_details?.lng != null;
...
exports.bulletDeliveryAPI = bulletDeliveryAPI;
exports.clearApiCache = clearApiCache;
exports.filterBranches = filterBranches;
exports.findNearestBranch = findNearestBranch;
exports.getGeolocationErrorMessage = getGeolocationErrorMessage;
exports.getIntentBranchId = getIntentBranchId;
exports.getIntentCityId = getIntentCityId;
exports.getIntentCountryCode = getIntentCountryCode;
exports.getIntentCountryId = getIntentCountryId;
exports.getIntentDistrictId = getIntentDistrictId;
exports.getIntentLatitude = getIntentLatitude;
exports.getIntentLongitude = getIntentLongitude;
exports.getIntentRegionId = getIntentRegionId;
exports.getIntentSubtitle = getIntentSubtitle;
exports.hasSessionAddressIntent = hasSessionAddressIntent;
exports.isBranchSelectable = isBranchSelectable;
exports.isMapOriginIntent = isMapOriginIntent;
exports.isSaudiArabia = isSaudiArabia;
exports.requireRegionAndDistrictForSA = requireRegionAndDistrictForSA;
