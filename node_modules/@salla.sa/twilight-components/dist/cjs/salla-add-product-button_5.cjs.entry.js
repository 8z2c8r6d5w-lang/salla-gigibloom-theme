...
var cart = require('./cart-DSvlJXCH.js');
var Helper = require('./Helper-DgRzYZwR.js');
var catalogRecommendations = require('./catalog-recommendations-Bxs1t6wk.js');
...
var applePayOrderOptions = require('./apple-pay-order-options-B036Wu4O.js');
...
const SallaAddProductButton = class {
    ...
    getLabel() {
        ...
        if (this.isIconOnlyLabel) {
            return `${cart.Cart}<span class="s-add-product-button-sr-only">${this.addToCartLabel}</span>`;
        }
        if (this.hasPreOrder) {
    ...
    get notifyChannels() {
        return this.channels || salla.api.product.getAvailabilityChannels?.() || Helper.Helper.availabilityChannels();
    }
...
const SallaBoughtTogether = class {
    ...
    componentWillLoad() {
        salla.onReady()
            ...
            return catalogRecommendations.fetchProductBoughtTogether(String(id));
        })
            .then((recommendedIds) => {
...
const SallaProductAvailability = class {
    ...
    channelsWatcher(newValue) {
        ...
        let channels = newValue || salla.api.product.getAvailabilityChannels?.() || Helper.Helper.availabilityChannels();
    ...
    render() {
        return (index.h(index.Host, { key: 'ba2423b2f12f2c6b3870c55b12157bbcf42407e8', class: "s-product-availability-wrap" }, this.isSubscribedToProduct
            ...
            :
                index.h("salla-button", { width: "wide", onClick: () => this.isUser ? this.submit() : this.openModel() }, salla.lang.get('pages.products.notify_availability')), this.isUser || this.isSubscribedToProduct ? '' : this.renderModal()));
    }
    renderModal() {
...
const SallaProductOptions = class {
    ...
    changedHandler(event, option, fireChangeEvent = true) {
        ...
        if (event.target.value ||
            ...
            (option.type === interfaces.DisplayType.MAP && event.type === 'selected' && Number.isFinite(Number(event.target.lat)) && Number.isFinite(Number(event.target.lng)))) {
            setTimeout(() => {
    ...
    hideDigitalCardsOptions(option) {
        return (this.disableCardValue && option.type === interfaces.DisplayType.DIGITAL_CARD_VALUE && !salla.url.is_page("cart"));
    }
    render() {
        if (this.optionsData?.length === 0) {
            return;
        }
        return (index.h(index.Host, { class: { 's-product-options-wrapper': true, 's-product-options-no-padding': this.noPadding } }, index.h("salla-conditional-fields", null, this.optionsData.map((option) => index.h("div", { key: option.id, class: `s-product-options-option-container${option.visibility_condition || this.hideDigitalCardsOptions(option) ? ' hidden' : ''}`, "data-option-id": option.id, ...this.getOptionShownWhen(option) }, option.name === 'splitter' ?
            this.splitterOption()
            : index.h("div", { class: { "s-product-options-option": true, "s-product-options-option-booking": option.type === interfaces.DisplayType.BOOKING && salla.url.is_page("cart") }, "data-option-type": option.type, "data-option-required": `${option.required}` }, index.h("label", { htmlFor: this.generateInputId(option.id), class: `s-product-options-option-label ${this.hideLabel(option) ? 's-product-options-option-label-hidden' : ''}` }, index.h("b", null, option.name, option.required && index.h("span", null, " * "), " "), index.h("small", null, option.placeholder)), index.h("div", { class: `s-product-options-option-content ${this.hideLabel(option) || (option.type === interfaces.DisplayType.BOOKING && salla.url.is_page("cart")) ? 's-product-options-option-content-full-width' : ''}` }, this.getDisplayForType(option))))))));
    }
    ...
    async componentDidLoad() {
        ...
        const selectedDonationOption = this.optionsData.find(option => option.type === interfaces.DisplayType.DONATION)?.details?.find(detail => detail.is_selected);
        ...
        if (!salla.url.is_page("cart") && !this.isCartMode) {
            const pricingOptionTypes = [
                interfaces.DisplayType.SINGLE_OPTION,
                interfaces.DisplayType.MULTIPLE_OPTIONS,
                interfaces.DisplayType.COLOR,
                interfaces.DisplayType.THUMBNAIL,
                interfaces.DisplayType.DONATION,
                interfaces.DisplayType.DIGITAL_CARD_VALUE,
                interfaces.DisplayType.COUNTRY
            ];
    ...
    fileUploader(option, additions = null) {
        return index.h("salla-file-upload", { ...(additions || {}), "payload-name": "file", value: option.value, "instant-upload": true, name: this.generateInputName(option.id), required: !option.visibility_condition && option.required, height: "120px", onAdded: (e) => {
    ...
    //@ts-ignore
    mapOption(option) {
        return index.h("salla-map", { zoom: 15, lat: this.getLatLng(option.value, 'lat'), lng: this.getLatLng(option.value, 'lng'), name: this.generateInputName(option.id), searchable: true, required: !option.visibility_condition && option.required, onInvalidInput: (e) => this.invalidHandler(e, option), onSelected: e => this.changedHandler(e, option) });
    }
    ...
    getOptionDetailName(detail, outOfStock = true, optionType, format) {
        let detailName;
        if (optionType && optionType === interfaces.DisplayType.COLOR) {
    ...
    renderInput(type, detail, option, isRequired, name, buttonStyle) {
        ...
        return (index.h("label", { class: {
            ...
            } }, index.h("input", { id: id, type: type, name: name, value: detail.id, disabled: this.canDisabled && isOutOfStock, required: isRequired, checked: detail.is_selected, onInvalid: (e) => this.invalidHandler(e, option), onChange: (e) => this.changedHandler(e, option) }), option.format === "price"
            ? index.h("div", { class: detailClass, innerHTML: detailName })
            : index.h("div", { class: detailClass }, detailName)));
    }
    /**
     ...
     */
    singleOption(option) {
        const buttonStyle = this.optionConfig?.['single-option']?.type === 'button' || (option.type === interfaces.DisplayType.SINGLE_OPTION && option.format === "price");
    ...
    //@ts-ignore
    colorOption(option) {
        return (index.h("fieldset", { class: "s-product-options-colors-wrapper" }, option?.details.map((detail) => {
            const isOutOfStock = this.isOptionDetailOut(detail);
            return (index.h("div", { class: {
                    "s-product-options-colors-item": true,
                    "s-product-options-option-stock-out": isOutOfStock,
                }, key: detail.id }, index.h("input", { type: "radio", value: detail.id, required: !option.visibility_condition && option.required, checked: detail.is_selected, name: this.generateInputName(option.id), disabled: this.canDisabled && isOutOfStock, id: this.generateUniqueKey(`color-${this.productId}-${option.id}-${detail.id}`), onInvalid: (e) => this.invalidHandler(e, option), onChange: (e) => this.changedHandler(e, option), "data-img-id": detail.option_value }), index.h("label", { htmlFor: this.generateUniqueKey(`color-${this.productId}-${option.id}-${detail.id}`), "data-img-id": detail.option_value }, index.h("span", { style: { backgroundColor: detail.color } }), index.h("div", { innerHTML: this.getOptionDetailName(detail, true, option.type) }))));
        })));
    }
    //@ts-ignore
    thumbnailOption(option) {
        return index.h("div", { class: "s-product-options-thumbnails-wrapper" }, option.details.map((detail) => {
            const isOutOfStock = this.isOptionDetailOut(detail);
            return index.h("div", { key: detail.id, class: { "s-product-options-option-stock-out": isOutOfStock } }, index.h("input", { type: "radio", value: detail.id, "data-itemid": detail.id, required: !option.visibility_condition && option.required, checked: detail.is_selected, name: this.generateInputName(option.id), "data-img-id": detail.option_value, disabled: this.canDisabled && isOutOfStock, id: this.generateUniqueKey(`option_${this.productId}-${option.id}_${detail.id}`), onInvalid: (e) => this.invalidHandler(e, option), onChange: (e) => this.changedHandler(e, option) }), index.h("label", { htmlFor: this.generateUniqueKey(`option_${this.productId}-${option.id}_${detail.id}`), "data-img-id": detail.option_value, class: "go-to-slide" }, index.h("img", { "data-src": detail.image, src: detail.image, title: detail.name, alt: detail.name }), index.h("span", { innerHTML: check.iconCheck, class: "s-product-options-thumbnails-icon" }), isOutOfStock ?
                [
                    index.h("small", { key: `badge-${detail.id}`, class: "s-product-options-thumbnails-stock-badge" }, this.outOfStockText),
                    this.canDisabled ? index.h("div", { key: `overlay-${detail.id}`, class: "s-product-options-thumbnails-badge-overlay" }) : '',
                ]
...
function setBaseCurrencyHeader() {
    index.axios.defaults.headers.common.currency = salla?.config?.get?.('user.currency_code');
}
    ...
    requestWithSupportAjax(url, payload, method = 'post') {
        return new Promise((resolve, reject) => {
            ...
            $.ajax({
                ...
                data: payload,
                headers: { currency: index.axios.defaults.headers.common.currency },
                async: false,
...
const SallaQuickBuy = class {
    constructor(hostRef) {
        ...
        this.handleApplePaySessionStarted = (session) => {
            ...
            applePayOrderOptions.resolveApplePayOrderOptions({
                productId: this.productId,
    ...
    validatePdpOptionsSync() {
        if (applePayOrderOptions.reportProductOptionsValiditySync(this.productId))
            return true;
    ...
    async settlePayment() {
        ...
        applePayOrderOptions.appendOrderOptionsToPayload(data, this.collectedOrderOptions);
    ...
    render() {
        return index.h(index.Host, { key: 'b03cdde677e52e751e8c1592a9da0ffc40614172' }, this.quickBuyButton());
    }
    quickBuyButton() {
...
exports.salla_bought_together = SallaBoughtTogether;
