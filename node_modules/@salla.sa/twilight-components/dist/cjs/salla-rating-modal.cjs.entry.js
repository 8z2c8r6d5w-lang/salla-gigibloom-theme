...
const SallaRatingModal = class {
    constructor(hostRef) {
        ...
        this.loyaltyPointsForImagesLabel = salla.lang.get('pages.rating.loyalty_points_for_images', { points: ':points' });
        ...
        salla.lang.onLoaded(() => {
            ...
            const setTranslations = async () => {
                ...
                await setNestedAsync('ar.trans', 'pages.rating.loyalty_points_for_images', 'اكسب :points نقطة ولاء عند إضافة صور لتقييمك!');
                await setNestedAsync('en.trans', 'pages.rating.loyalty_points_for_images', 'Earn :points loyalty points when adding photos to your review!');
    ...
    getV2Ref() {
        return this.v2Ref ?? this.host.querySelector('salla-rating-modal-v2');
    }
    syncNewPopUpFlag() {
        // Enable the new pop-up to all consumers:
        this.enableNewPopUp = true;
    }
    /**
    ...
    async setOrder(orderId, productId) {
        ...
        await this.getV2Ref()?.setOrder(orderId, productId);
    }
    /**
    ...
    async open() {
        await salla.onReady();
        this.syncNewPopUpFlag();
        if (this.enableNewPopUp) {
            return this.getV2Ref()?.open();
        }
        this.defaultMode = true;
        const requestedOrderId = this.orderId ? this.orderId : salla.config.get('page.id');
        return new Promise((resolve) => {
            setTimeout(() => {
                this.modal.open().then(() => (this.order && this.order.id === requestedOrderId) || salla.api.withoutNotifier(() => salla.rating.api.order(requestedOrderId)).then(res => this.order = res.data))
    ...
    async openEditModal(params) {
        ...
        this.syncNewPopUpFlag();
        if (this.enableNewPopUp) {
            return this.getV2Ref()?.openEditModal(params);
        }
        this.defaultMode = false;
        this.editMode = true;
    ...
    async fetchEditItem(feedbackId) {
        ...
        if (this.allowAttachImages && this.editType == "product" && this.editItem.images.length) {
            this.editItemImages = this.editItem.images.map(img => ({ id: img, url: img }));
        }
    }
    async editReview() {
        const data = {};
    ...
    async openDeleteModal(params) {
        ...
        this.syncNewPopUpFlag();
        if (this.enableNewPopUp) {
            return this.getV2Ref()?.openDeleteModal(params);
        }
        this.defaultMode = false;
    ...
    async readyUpLoyaltyPoints() {
        ...
        this.loyaltyImageRatingPoints = Number(rating_product_with_image_points) + Number(rating_product_points);
    }
    loyaltyPointsNote() {
        if (!this.loyaltyImageRatingPoints || !this.loyaltyPointsForImagesLabel) {
            return null;
        }
    ...
    render() {
        if (this.enableNewPopUp) {
            return (index.h(index.Host, { class: "s-rating-modal" }, index.h("salla-rating-modal-v2", { ref: (el) => (this.v2Ref = el), orderId: this.orderId, productId: this.productId })));
        }
        if (this.deleteMode) {
        ...
        if (this.defaultMode) {
            return (index.h(index.Host, { class: "s-rating-modal" }, index.h("salla-modal", { class: "s-rating-modal-wrap", onModalVisibilityChanged: (isOpen) => this.resetData(isOpen), isLoading: true, width: "md", ref: modal => this.modal = modal }, index.h("div", { slot: 'loading' }, index.h("div", { class: "s-rating-modal-skeleton" }, index.h("salla-skeleton", { type: 'circle', height: '80px', width: '80px' }), index.h("salla-skeleton", { height: '15px', width: '60%' }), index.h("salla-skeleton", { height: '10px', width: '30%' }), index.h("div", { class: "s-rating-modal-skeleton-stars" }, [...Array(5)].map((v) => index.h("div", { key: v, innerHTML: star.Star }))), index.h("salla-skeleton", { height: '100px', width: '100%' }), index.h("div", { class: "s-rating-modal-skeleton-footer" }, index.h("salla-skeleton", { height: '40px', width: '30%' })))), !this.hasError && this.order
                ? [index.h("div", { class: "s-rating-modal-wrapper", ref: el => this.body = el }, this.order?.testimonials_enabled ?
                        ...
                        ? index.h("div", { class: "rating-outer-form s-rating-modal-step-wrap s-rating-modal-step s-rating-modal-hidden", "data-type": "shipping" }, index.h("input", { type: "hidden", name: "shipping_company_id", value: this.order.shipping.company.id }), this.order.shipping.company.logo
                            ...
                            : index.h("span", { class: "s-rating-modal-icon", innerHTML: star.ShippingFast }), index.h("div", { class: "s-rating-modal-title" }, " ", `${salla.lang.get('pages.rating.rate_shipping')} ${this.order.shipping.company.name}`), index.h("div", { class: "s-rating-modal-stars-company" }, index.h("salla-rating-stars", { withLabel: true, size: "large" })), index.h("textarea", { name: "comment", class: "s-rating-modal-comment", placeholder: salla.lang.get('pages.rating.write_shipping_rate') }), index.h("small", { class: "s-rating-modal-validation-msg" }))
    ...
    async componentWillLoad() {
        salla.event.on('rating::open', (params) => {
            const orderId = params?.orderId || salla.config.get('page.id');
            this.setOrder(orderId, params?.productId).then(() => this.open());
        });
