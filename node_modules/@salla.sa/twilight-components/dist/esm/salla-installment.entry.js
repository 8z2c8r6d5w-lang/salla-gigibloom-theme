...
const SallaInstallment = class {
    ...
    async componentWillLoad() {
        ...
        if (installment) {
            ...
            this.klarnaIsActive = installment.klarna;
        }
    ...
    render() {
        ...
        return (h(Host, null, this.tamaraIsActive &&
            ...
            (this.mispayActive.publicKey ? (h("div", { class: "mb-5" }, h("mispay-widget", { amount: this.price, lang: this.language }))) : (h("div", { class: "s-installment-mispay-wrapper" }, h("div", { class: "s-installment-mispay-content" }, h("img", { src: salla.url.cdn('images/payment/png/mispay.png'), alt: "mispay" }), h("span", { class: "s-installment-mispay-content-text", innerHTML: this.installment_sheria_text('pages.products.installments', {
                    ...
                    installments: 4,
                }) }))))), this.emkanIsActive && this.currency === "SAR" ? (h("div", { class: "s-installment-emkan-wrapper" }, h("div", { class: "s-installment-emkan-content" }, h("img", { src: salla.url.assetsCdn('images/payment/png/emkan.png'), alt: "emkan" }), h("span", { class: "s-installment-emkan-content-text", innerHTML: this.installment_sheria_text('pages.products.installments_emkan', {
                payment: salla.money(parseFloat(this.price)),
                installments: 5,
            ...
            : '', this.klarnaIsActive && this.klarnaIsActive.locales?.length > 0 && (h("div", { class: "s-installment-klarna-wrapper" }, h("klarna-placement", { "data-key": this.klarnaIsActive.dataKey || 'credit-promotion-badge', "data-locale": this.getKlarnaLocale(), "data-purchase-amount": String(Math.round(parseFloat(this.price) * 100)) })))));
    }
    renderInstallments(isUpdating = false) {
        ...
        // Klarna
        if (this.klarnaIsActive && this.klarnaIsActive.locales?.length > 0) {
            this.loadKlarna(isUpdating);
        }
        // Mispay
    ...
    loadKlarna(isUpdating) {
        window.KlarnaOnsiteService = window.KlarnaOnsiteService || [];
        if (isUpdating) {
            const placement = this.host.querySelector('klarna-placement');
            if (placement) {
                placement.setAttribute('data-purchase-amount', String(Math.round(parseFloat(this.price) * 100)));
                window.KlarnaOnsiteService.push({ eventName: 'refresh-placements' });
            }
            return;
        }
        var oldKlarnaScript = document.querySelector('script[src="https://js.klarna.com/web-sdk/v1/klarna.js"]');
        if (oldKlarnaScript) {
            window.KlarnaOnsiteService.push({ eventName: 'refresh-placements' });
            return;
        }
        ...
        klarnaScript.setAttribute('data-client-id', this.klarnaIsActive.clientId);
        klarnaScript.onload = () => {
            window.KlarnaOnsiteService.push({ eventName: 'refresh-placements' });
        };
    ...
    getKlarnaLocale() {
        const locales = this.klarnaIsActive?.locales || [];
        if (!locales.length)
            return null;
        const byLang = new Map(locales?.map(l => [l.split('-')[0], l]));
        return byLang.get(this.language) ?? byLang.get('en') ?? locales[0];
    }
    shouldShowMispay() {
