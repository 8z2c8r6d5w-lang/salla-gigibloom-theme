 ...
 */
import { r as registerInstance, h, F as Fragment, a as getElement } from './index-BpFlqPIJ.js';

const sallaLoginModalCss = "/*!twlt*/iframe{-moz-transition:height 0.3s;-ms-transition:height 0.3s;-o-transition:height 0.3s;-webkit-transition:height 0.3s;transition:height 0.3s;width:100%}.inline{display:inline}.filter{filter:var(--tw-blur) var(--tw-brightness) var(--tw-contrast) var(--tw-grayscale) var(--tw-hue-rotate) var(--tw-invert) var(--tw-saturate) var(--tw-sepia) var(--tw-drop-shadow)}";
...
const SallaLoginModal = class {
    constructor(hostRef) {
        ...
        salla.event.on('login::open', (data) => this.onLoginOpen(data));
    ...
    /**
     * `login::open` handler. Newer non-SPA native apps (>= 5.0.38) open their OWN native login
     * screen — the mobile bridge emits `mobile::login.open` — so keep the web modal closed to avoid
     * stacking two logins. Extracted from the constructor so it is unit-testable via the prototype
     * (the constructor-based spec suite is quarantined; see salla-login-modal.spec.ts).
     */
    onLoginOpen(data) {
        if (salla.mobile?.isNativeLoginEnabled?.())
            return;
    ...
    componentDidLoad() {
        window.addEventListener('message', (event) => {
            // Trust only our login iframe/passkey popup at the accounts origin.
            if (!this.isTrustedMessage(event) || event.data?.source != "login") {
                return;
            }
            ...
            if (action == 'ready') {
                if (value.isPasskeyPage) {
                    this.createPassKey.postMessage({
                        source: "login", type: "init_passkey_page", data: {
    ...
    // Accept postMessage only from our own login iframe/passkey popup at the accounts origin.
    isTrustedMessage(event) {
        const fromIframe = !!this.iframe && event.source === this.iframe.contentWindow;
        const fromPasskey = !!this.createPassKey && event.source === this.createPassKey;
        if (!fromIframe && !fromPasskey)
            return false;
        let origin;
        try {
            origin = new URL(salla.config.get('login.url', 'https://accounts.salla.com')).origin;
        }
        catch (e) {
            origin = 'https://accounts.salla.com';
        }
        return event.origin === origin;
    }
    // Reject javascript:/data: and other non-http(s) navigation targets.
    isSafeUrl(url) {
    ...
    async handleGuestCheckout() {
        this.modal.close();
        if (salla.config.isGuest()) {
            return salla.cart.submit(false, true);
        }
    }
    openModal() {
    ...
    render() {
        //let's avoid showing the login modal if the user is already logged in
    ...
    getLoginDom() {
        ...
        return h(Fragment, null, h("iframe", { src: `${salla.config.get('login.url', 'https://accounts.salla.com')}?src=${this.source}&version=${salla.config.get('login.version', '0.1.78')}`, title: "Salla Login", class: "hide-scroll", loading: 'lazy', ref: iframe => { this.iframe = iframe; }, allow: "publickey-credentials-get; publickey-credentials-create", scrolling: this.scrolling, frameborder: "0", width: "100%" }));
    }
