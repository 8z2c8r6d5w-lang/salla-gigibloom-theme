...
const sallaMapCss = "/*!twlt*/.s-map-wrapper{position:relative}.s-map-value-input{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;white-space:nowrap;border:0;opacity:0;pointer-events:none}.s-map-wrapper.s-has-error .s-map-location-button{outline:2px solid rgb(248, 113, 113);outline-offset:2px}.visible{visibility:visible}.s-has-error{--tw-border-opacity:1;border-color:rgb(248 113 113 / var(--tw-border-opacity, 1))}.s-has-error:focus{--tw-border-opacity:1;border-color:rgb(239 68 68 / var(--tw-border-opacity, 1))}.s-has-error{--tw-border-opacity:1 !important;border-color:rgb(239 68 68 / var(--tw-border-opacity, 1)) !important;--tw-text-opacity:1 !important;color:rgb(239 68 68 / var(--tw-text-opacity, 1)) !important}.s-map-modal-title{position:relative;margin-top:1rem;margin-bottom:1rem;padding-left:1rem;padding-right:1rem}.s-map-location-icon{margin-right:0.5rem}.s-map-location-icon:where([dir=\"rtl\"],[dir=\"rtl\"] *){margin-left:0.5rem;margin-right:0px}.s-map-location-icon svg{height:1rem !important;width:1rem !important}.s-map-modal-body{position:relative;overflow:hidden}.s-map-element{height:400px;width:100%}.s-map-search-wrapper{position:absolute;top:0px;z-index:1;margin:1.25rem;width:-webkit-fill-available}.s-map-search-wrapper input{height:2.5rem;width:100%;padding:0.5rem}.s-map-search-wrapper input:where([dir=\"rtl\"],[dir=\"rtl\"] *){text-align:right}.s-map-search-results{margin-top:0px;display:flex;height:6rem;flex-direction:column;padding:0.5rem}.s-map-modal-body .s-loading-container{position:absolute;margin:0px;padding:5rem}.s-map-my-location-button{position:absolute;bottom:1.5rem;left:0px;margin:0.5rem}.s-map-location-button .s-button-text{display:flex;align-items:center}.s-map-submit-button button{border-top-right-radius:0px;border-top-left-radius:0px;padding-top:12px;padding-bottom:12px}";
...
const SallaMap = class {
    constructor(hostRef) {
        ...
        salla.onReady(() => {
            this.apiKey = salla.config.get('store.settings.keys.maps');
        });
    }
    formatAddress(address) {
        return address.length > 25 ? address.substring(0, 25) + '...' : address;
    }
    /** Treat numeric `0` as valid; reject null/undefined/blank/NaN (incl. whitespace). */
    hasCoordinate(value) {
    ...
    /**
     * Prefer confirmed selection; fall back to lat/lng props so cart/order-form
     * options with an existing location keep a valid form value before geocode finishes.
     */
    getLatLng() {
        const lat = this.hasCoordinate(this.selectedLat) ? this.selectedLat : this.lat;
        const lng = this.hasCoordinate(this.selectedLng) ? this.selectedLng : this.lng;
        return this.hasCoordinate(lat) && this.hasCoordinate(lng) ? `${lat}, ${lng}` : '';
    }
    syncMapInputValue() {
        if (!this.mapInput) {
            return;
        }
        this.mapInput.value = this.getLatLng();
        this.mapInput.setCustomValidity('');
    }
    focusLocateControl() {
        const locateButton = this.host.querySelector('.s-map-location-button');
        locateButton?.focus?.();
    }
    componentWillLoad() {
        if (this.hasCoordinate(this.lat) && this.hasCoordinate(this.lng)) {
    ...
    componentDidLoad() {
        ...
        this.mapInput.addEventListener('invalid', e => {
            // Keep native constraint validation, but move focus to the visible control
            // so Chromium does not throw "An invalid form control is not focusable".
            e.preventDefault();
            this.host.classList.add('s-has-error');
            this.focusLocateControl();
            this.invalidInput.emit(e);
        });
        this.mapInput.addEventListener('input', () => {
            this.mapInput.setCustomValidity('');
            this.host.classList.remove('s-has-error');
        });
    }
    /**
     * Validate the selected location when the field is required.
     * Native required still applies on a focusable visually-hidden input; this method
     * syncs value first and routes focus to the visible locate control via `invalid`.
     */
    async reportValidity() {
        this.syncMapInputValue();
        if (!this.mapInput || !this.required || this.readonly || this.getLatLng()) {
            this.mapInput?.setCustomValidity('');
            this.host.classList.remove('s-has-error');
            return true;
        }
        return this.mapInput.reportValidity();
    }
    /**
    ...
    // render
    render() {
        return (h(Host, { key: 'a1c17b249dca7f6bf172e235b1960c634fba4e02', class: "s-map-wrapper" }, h("salla-modal", { key: 'd896fd882de7598dfe9ea1fddff571d5bd43b8ec', class: "s-map-modal-wrapper", noPadding: true, ref: modal => {
                this.locationModal = modal;
            } }, this.getLocationModal()), h("slot", { key: 'bf70602eba04f09ea904c0cc4cdda37ef9bc3988', name: "button" }, h("salla-button", { key: 'e9ab67288c8bc4a8ffeff9f49683b57421aaaaef', onClick: () => {
                this.open();
            }, color: "primary", class: "s-map-location-button" }, h("span", { key: '9e8d2a788cf5e209f00b73280b5b54701f354171', class: "s-map-location-icon", innerHTML: this.formattedAddress ? Edit : Location }), this.formattedAddress ? (h("div", null, this.locateButtonEdit, " | ", this.formatAddress(this.formattedAddress))) : (this.locateButtonTitle))), h("input", { key: '8c07eaa39a6f881147bda89f683e1ae3d9fc0a0f', class: "s-map-value-input", name: this.name, required: this.required, value: this.getLatLng(), tabIndex: -1, "aria-hidden": "true", ref: input => (this.mapInput = input) })));
    }
    get host() { return getElement(this); }
};
SallaMap.style = sallaMapCss;

const sallaProgressBarCss = "/*!twlt*/.s-progress-bar-container{margin-bottom:0.625rem}.s-progress-bar-header{margin-bottom:0.375rem;font-size:0.875rem;line-height:1.25rem;font-weight:700;--tw-text-opacity:1;color:rgb(107 114 128 / var(--tw-text-opacity, 1))}.s-progress-bar-target-section{margin-bottom:0.625rem;display:flex;justify-content:space-between;font-size:0.875rem;line-height:1.25rem}.s-progress-bar-wrapper{margin-bottom:0.375rem;width:100%;border-radius:9999px;--tw-bg-opacity:1;background-color:rgb(229 231 235 / var(--tw-bg-opacity, 1))}.s-progress-bar-wrapper:is(.dark *){--tw-bg-opacity:1;background-color:rgb(55 65 81 / var(--tw-bg-opacity, 1))}.s-progress-bar-progress{height:100%;border-radius:9999px;--tw-bg-opacity:1;background-color:rgb(37 99 235 / var(--tw-bg-opacity, 1))}.s-progress-bar-message{margin-bottom:0.625rem;display:block;font-size:0.875rem;line-height:1.25rem;--tw-text-opacity:1;color:rgb(248 113 113 / var(--tw-text-opacity, 1))}.s-progress-bar-wrapper .s-progress-bar-progress-stripped{background-image:linear-gradient(-45deg, rgba(255, 255, 255, .2) 25%, transparent 25%, transparent 50%,rgba(255, 255, 255, .2) 50%,rgba(255, 255, 255, .2) 75%, transparent 75%, transparent);background-size:30px 30px}.s-reviews-summary-progress .s-progress-bar-wrapper{margin-bottom:0px}.s-reviews-summary-progress .s-progress-bar-container{margin-bottom:0px}";
    ...
    async componentWillLoad() {
        ...
        if (this.listenToThumbnailsOption) {
            salla.event.on('product-options::change', data => {
                if (!['thumbnail', 'color'].includes(data?.option?.type)) {
                    return;
                }
                // Options with their own uploaded thumbnail carry the gallery image id in `color`.
                const imageIds = [data?.detail?.option_value, data?.detail?.color]
                    .filter(value => value != null && /^\d+$/.test(String(value)));
                for (const imageId of imageIds) {
                    const slideIndex = this.sliderWrapper
                        ?.querySelector(`[data-img-id="${CSS.escape(String(imageId))}"]`)
                        ?.getAttribute('data-slid-index');
                    if (slideIndex != null) {
                        this.slideTo(parseInt(slideIndex, 10), 300, false);
                        return;
                    }
                }
            });
...
export { SallaMap as salla_map, SallaProgressBar as salla_progress_bar, SallaSlider as salla_slider };
