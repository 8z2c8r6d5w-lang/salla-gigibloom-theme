...
const SallaTieredOffer = class {
    ...
    shouldHideUntilThreshold(details, tierIndex) {
        return !!(details?.hide_until_threshold && tierIndex === 0);
    }
    resolveTierName(tiers, index) {
        return TierUtils.getTierNameByIndex(tiers, index) || '';
    }
    ...
    initializeComponent(apiOfferData) {
        ...
        // Hide entire widget for unauthenticated users
        if (this.shouldHideForGuest()) {
            this.canRender = false;
            return;
        }
        // Only hide until threshold for merchants with the flag enabled (e.g. Arabian Oud)
        if (this.shouldHideUntilThreshold(this.apiOfferData.details, currentTierIndex)) {
            this.canRender = false;
            return;
        }
        this.currentTierIndex = currentTierIndex;
        this.currentTier = this.resolveTierName(this.tieredOfferData.tiers, currentTierIndex);
        // Default to first tab so tabs always have visible content
        this.activeTabTier = this.currentTier || this.tieredOfferData.tiers[0]?.tier_name;
        this.canRender = true;
    }
    componentDidLoad() {
    ...
    updateOfferValues(updatedCart, freshOfferData) {
        ...
        if (!freshOfferData.details?.discounts?.length) {
            this.canRender = false;
            return;
        }
        ...
        // Build fresh tier map so index computation uses up-to-date thresholds
        const freshTieredData = this.mapApiDataToTiers(freshOfferData);
        const currentValue = freshOfferData.details?.current_value || 0;
        const newTierIndex = this.findCurrentTierIndex(currentValue, freshTieredData.tiers);
        // Only hide until threshold for merchants with the flag enabled (e.g. Arabian Oud)
        if (this.shouldHideUntilThreshold(freshOfferData.details, newTierIndex)) {
            this.canRender = false;
            return;
        }
        // Offer progress is based on historical purchases only — skip state update on cart changes
        // Use strict === false to avoid treating an omitted flag as false
        if (freshOfferData.details?.with_current_cart === false) {
            this.canRender = true;
            return;
        }
        // All guards passed — update state atomically
        this.apiOfferData = freshOfferData;
        this.tieredOfferData = freshTieredData;
        this.canRender = true;
        this.currentTierIndex = newTierIndex;
        // Tiers are 1-based; fall back to first tier when no tier has been reached yet
        this.currentTier = this.resolveTierName(freshTieredData.tiers, newTierIndex);
        // Update active tab if needed; default to first tab when no tier reached
        const resolvedTab = this.currentTier || freshTieredData.tiers[0]?.tier_name;
        if (this.activeTabTier !== resolvedTab) {
            this.activeTabTier = resolvedTab;
        }
    }
    ...
    render() {
        ...
        return (h(Host, null, h("div", { class: "s-tiered-offer-container" }, h("div", { class: "s-tiered-offer-header" }, h("div", { class: "s-tiered-offer-header-content" }, h("p", { class: "s-tiered-offer-program-title" }, this.loyaltyProgramText), this.currentTier && (h("div", { class: "s-tiered-offer-current-tier-name-wrapper" }, h("h2", { class: "s-tiered-offer-current-tier" }, this.currentTierText, " ", h("span", { class: "s-tiered-offer-tier-name" }, currentTierData.name)), h("span", { innerHTML: currentTierData.icon }))))), this.renderProgressBar(), this.renderTabs())));
    }
};
