...
const SallaUserMenu = class {
    constructor(hostRef) {
        ...
        this.loyalty_points = salla.lang.get('pages.loyalty_program.loyalty_points');
        ...
        // Fetched once per page load; resets on remount (page refresh), not on open.
        this.walletBalanceLoaded = false;
        ...
        this.OrderUpdate = 0;
        // STD-13103: these pages moved under /account; link directly to avoid 301 redirects.
        this.accountPrefixedKeys = ['orders', 'pending_orders', 'wallet', 'notifications', 'rating', 'profile', 'wishlist', 'settings'];
        this.items = {
            ...
            wishlist: Heart,
            wallet: WalletIcon,
            loyalty_points: loyaltyProgramIcon,
            profile: UserCircle
        };
    }
    ...
    async loadWalletBalance() {
        // Live balance instead of the cached (stale) config; light skips transactions.
        if (Salla.config.isGuest())
            return;
        this.walletBalanceLoading = true;
        try {
            // withoutNotifier: a failed background fetch must not toast the shopper.
            const resp = await salla.api.withoutNotifier(() => salla.api.request('/balance/wallet?light=1'));
            const balance = Number(resp?.data?.balance) || 0;
            // Currency stays from config — it doesn't go stale like the amount.
            this.walletBalance = balance
                ? Salla.money({ amount: Number(balance.toFixed(2)), currency: Salla.config.get('user.currency_code') })
                : '';
            this.walletBalanceLoaded = true; // cache for the rest of this page load
        }
        catch {
            // keep the last value on failure (retries on next open)
        }
        finally {
            this.walletBalanceLoading = false;
        }
    ...
    async open(e) {
        ...
        e.stopPropagation();
        if (this.opened) {
            // Fetch once per page load; reuse the cached value on later opens.
            if (window.can_access_wallet && !this.walletBalanceLoaded && !this.walletBalanceLoading) {
    ...
    getItemAnchorLinkAttrs(itemKey) {
        ...
        const urlKey = itemKey === 'loyalty_points' ? 'loyalty' : itemKey;
        const path = this.accountPrefixedKeys.includes(urlKey) ? `account/${urlKey}` : urlKey;
        return { href: Salla.url.get(path) };
    }
    getMenuItem([itemKey, itemValue], i) {
        //todo:: enhancement support slot here
        if (itemKey === 'wallet' && !window.can_access_wallet)
            return;
        if (itemKey === 'loyalty_points' && !Salla.config.get('store.features').includes('loyalty-system'))
            return;
        return (h("li", { class: {
                's-user-menu-dropdown-item': true,
                's-user-menu-dropdown-item-logout': i + 1 === Object.entries(this.items).length,
            } }, h("a", { ...this.getItemAnchorLinkAttrs(itemKey), class: "s-user-menu-dropdown-item-link", onClick: event => this.menuItemClicked(event, itemKey) }, h("i", { class: "s-user-menu-dropdown-item-prefix", innerHTML: itemValue }), h("span", { class: "s-user-menu-dropdown-item-title" }, this[itemKey]), itemKey === 'wallet' && !this.inline && (this.walletBalance || this.walletBalanceLoading) ? (h("span", { class: "s-user-menu-dropdown-item-balance", style: { display: 'inline-flex', alignItems: 'center', gap: '4px' } }, this.walletBalance ? h("span", { innerHTML: this.walletBalance }) : '', this.walletBalanceLoading ? h("salla-loading", { size: "14" }) : '')) : (''), !['٠', '0', undefined].includes(this.badges[itemKey]) ? (h("span", { class: "s-user-menu-dropdown-item-badge" }, this.badges[itemKey])) : (''))));
    }
    ...
    async componentDidLoad() {
        await Salla.hooks.registerComponent('salla-user-menu', this);
