...
export interface IntentAddressDetails {
    ...
    short_address?: string;
    /** Map-origin pick: coordinates dropped on the map (no city/district resolved client-side) */
    lat?: number;
    lng?: number;
    /** Map-origin pick: human-readable reverse-geocoded address */
    formatted_address?: string;
    /** Map-origin pick: Google Place ID (not a display string) */
    place_id?: string;
    /** Guest-entered address description (map flow only) */
    description?: string;
}
/**
...
export interface BulletDeliveryResult {
    ...
    /**
     * True only once scope allocation has actually succeeded for this intent — distinct from
     * address_id/branch_id, which can be set as soon as the address is *saved* (before
     * allocation even runs), so a later allocation failure can't make the intent look complete.
     */
    scope_allocated?: boolean;
}
/**
 * Event payloads
 */
...
export interface BulletDeliveryAllocationConfirmedAddressData {
    ...
    short_address?: string;
    form_data?: NewAddressFormData;
    lat?: number;
    lng?: number;
    formatted_address?: string;
}
export interface BulletDeliveryAllocationConfirmedEvent extends BulletDeliveryResult {
    address: BulletDeliveryAllocationConfirmedAddressData | null;
    branch?: Branch | null;
    allocation_request: ScopeAllocationPayload;
    allocation_response: ScopeAllocationResponse | null;
}
 ...
 */
export interface ScopeAllocationPayload {
    type: 'address' | 'branch';
    country_id?: string;
    region_id?: string;
    city_id?: string;
